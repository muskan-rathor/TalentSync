...
export const updateProfile=TryCatch(async(req:AuthenticatedRequest,res,next)=>{
    ...
    UPDATE users SET name=${newName},phone_number=${newPhoneNumber},bio=${newBio} WHERE user_id=${user.user_id} RETURNING user_id,name,email,phone_number,role,bio
    `;
...
export const updateProfilePic=TryCatch(async(req:AuthenticatedRequest,res)=>{ 
    ...
    const file=req.file;
    if(!file){
        throw new ErrorHandler(400,"No imagefile provided");
    }
  ...
  `${process.env.UPLOAD_SERVICES_URL}/api/utils/upload`,
  {
 ...
 export const addSkillToUser=TryCatch(async(req:AuthenticatedRequest,res)=>{
    const userId=req.user?.user_id; 
     ...
     try{
        ...
        console.log("Skill coming from request:", skillName);
 ...
 export const deleteSkillFromUser=TryCatch(async(req:AuthenticatedRequest,res)=>{  
    ...
    if(!skillName||skillName.trim().length===0){
     ...
     if(result.length===0){
      ...
      res.json({
        message:`Skill ${skillName.trim()} deleted from user successfully`,
      });
 });
 ...
 export const applyForJob=TryCatch(async(req:AuthenticatedRequest,res)=>{
    ...
    if(user.role!=="jobseeker"){
        throw new ErrorHandler(403,"Forbidden: Only candidates can apply for jobs");
    }
    ...
    let newApplication;
    try{
        [newApplication]=await sql `INSERT INTO applications (applicant_id,job_id,applicant_email,resume,subscribed) VALUES (${applicant_id},${job_id},${user?.email},${resume},${isSubscribed})`;
    }
    catch(error:any){
        if(error.code==="23505"){
